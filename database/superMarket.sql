create or replace DATABASE superMarket;
USE superMarket;
...
create or replace TABLE foodSection(
	productID INT(4),
...
create or replace TABLE household(
	productID INT(4),
	quantity VARCHAR(10) NOT NULL default ' - ',
...
create or replace TABLE clothes(
	...
	season VARCHAR(20) CHECK (season IN("Summer Wear", "Winter Wear", "Monsoon Wear", "All Time")),
	PRIMARY KEY (productID),
	FOREIGN KEY (productID) REFERENCES product(productID)
	ON DELETE CASCADE
	ON UPDATE CASCADE
...
create or replace TABLE supplied(
...
create or replace TABLE discount(
	...
	discountPercent DECIMAL(4, 2) NOT NULL CHECK(discountPercent > 0),
